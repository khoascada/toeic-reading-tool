...
import { submitPart6Attempts, getPart6PassageById } from '@/repositories';
import { SubmitPart6Payload } from '@/types/part.type';

export async function POST(request: NextRequest) {
  try {
    const body: SubmitPart6Payload = await request.json();
    ...
    const passage = await getPart6PassageById(passageId);
    if (!passage) {
    ...
    const result = await submitPart6Attempts(body);
    return NextResponse.json(result);
  } catch (error) {
    console.error('Error submitting Part 6 attempts:', error);
    return NextResponse.json({ message: 'Lỗi khi lưu kết quả bài làm Part 6' }, { status: 500 });
  }
}
