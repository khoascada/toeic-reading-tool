...
interface Part6StartDropdownProps {
  className?: string;
  size?: 'default' | 'sm' | 'lg';
}

export const Part6StartDropdown: React.FC<Part6StartDropdownProps> = ({
  className,
  size = 'lg',
}) => {
  ...
  const handleSelectStatus = async (status: Part6StatusFilter) => {
    const randomId = await getRandomId(status);
    if (randomId) {
      router.push(`/part-6/${randomId}?status=${status}`);
    } else {
