...
import { useGetPart7Stats } from '../hooks';
import { Part7StartDropdown } from './part-7-start-dropdown';
...
export const Part7DashboardView = () => {
  const { data: stats, isLoading, isError } = useGetPart7Stats();

  if (isLoading) {
    return (
      <div className="container mx-auto max-w-5xl px-4 py-20 flex flex-col items-center justify-center min-h-[400px]">
        <Loader2 className="h-8 w-8 animate-spin text-primary mb-3" />
        <p className="text-sm font-medium text-muted-foreground">Đang tải thống kê Part 7...</p>
      </div>
    );
  }
  ...
  if (isError || !stats) {
    return (
      <div className="container mx-auto max-w-5xl px-4 py-16 text-center">
        <p className="text-sm text-destructive font-semibold">Không thể tải dữ liệu thống kê Part 7.</p>
  ...
  return (
    <div className="container mx-auto max-w-5xl px-4 py-6 md:py-10 space-y-8">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-primary/20 bg-gradient-to-br from-primary/10 via-background to-muted/40 p-6 md:p-8 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2.5 max-w-2xl">
            <div className="inline-flex items-center gap-1.5 rounded-full bg-primary/15 px-3 py-1 text-xs font-semibold text-primary border border-primary/20">
              ...
              <span>TOEIC Reading • Reading Comprehension</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-foreground">
              Part 7: Đọc hiểu đoạn văn
            </h1>
            <p className="text-muted-foreground text-sm leading-relaxed">
              Phần thi gồm các bài đọc đơn và đôi (email, bài báo, thông báo, biểu đồ...). Mỗi đoạn kèm theo
              2–5 câu hỏi về nội dung, chi tiết, từ vựng và suy luận.
            </p>
          </div>

          <div className="shrink-0">
            <Part7StartDropdown size="lg" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-6 mt-6 border-t border-border/50">
          <div className="flex items-center gap-3 bg-background/70 rounded-xl p-3 border border-border/40">
            ...
            <div>
              <div className="text-xs font-bold text-foreground">54 câu hỏi / đề</div>
              <div className="text-[11px] text-muted-foreground">Câu 147 đến câu 200</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 bg-background/70 rounded-xl p-3 border border-border/40">
            ...
            <div>
              <div className="text-xs font-bold text-foreground">10 đoạn đơn + 5 đoạn đôi</div>
              <div className="text-[11px] text-muted-foreground">2–5 câu hỏi mỗi đoạn</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 bg-background/70 rounded-xl p-3 border border-border/40">
            ...
            <div>
              <div className="text-xs font-bold text-foreground">~55 phút</div>
              <div className="text-[11px] text-muted-foreground">Khoảng 1 phút / câu</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* 3. Danh sách các đoạn văn Part 7 */}
